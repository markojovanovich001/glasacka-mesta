...
layout_start('Check', 'check', [['Check', null]]);
?>
...
<div class="table-card mb-4">
  <div class="table-card-header flex-wrap gap-2">
    ...
    <span class="fw-semibold">Check terminal</span>
    ...
    <div class="ms-auto d-flex gap-2 align-items-center flex-wrap">
      ...
      <button id="btnFirstRun"
              class="btn btn-sm btn-primary d-flex align-items-center gap-1"
              onclick="startScraper()">
        <i class="bi bi-search"></i>
        <span>Pokreni check</span>
      </button>
      <button id="btnStop"
              class="btn btn-sm btn-danger d-flex align-items-center gap-1"
              onclick="stopScraper()" disabled>
  ...
  <div id="progressWrap" style="background:#0d1117;padding:8px 16px 10px;display:none;
    ...
    <div class="d-flex gap-4 mt-2">
      <span style="font-size:.71rem;font-family:monospace;color:#6ee7b7;">
        <i class="bi bi-arrow-down-circle me-1"></i>Nova: <strong id="sNova">0</strong>
      </span>
  ...
  <div style="padding:8px 16px;background:#1e293b;border-top:1px solid #0f172a;
    ...
    <?php if ($logSize > 0): ?>
      ...
      <?php endif; ?>
    <?php else: ?>
      <i class="bi bi-journal-x me-1"></i>Log je prazan &ndash; scraper još nije pokrenut.
    <?php endif; ?>
  </div>
</div>

<!-- ── Opasne radnje ──────────────────────────────────────── -->
<div class="table-card" style="border-color:#fca5a5;">
  <div class="table-card-header" style="background:#fff5f5;">
    <i class="bi bi-exclamation-triangle-fill text-danger"></i>
    <span class="fw-semibold text-danger">Opasne radnje</span>
  </div>
  <div class="p-4 d-flex align-items-start gap-4 flex-wrap">
    <div style="max-width:560px;">
      <strong>Resetuj bazu i storage</strong>
      <p class="mb-0 text-muted mt-1" style="font-size:.85rem;">
        Briše sve iz tabela <code>rik_fajlovi</code>, <code>rik_fajlovi_verzije</code>,
        <code>glasacka_mesta</code> i sve preuzete fajlove iz <code>storage/fajlovi/</code>.
        Tabele <code>opstine</code> i <code>gradovi</code> ostaju netaknute.
        Koristi za potpuni restart od nule.
      </p>
    </div>
    <button class="btn btn-outline-danger btn-sm ms-auto align-self-center"
            data-bs-toggle="modal" data-bs-target="#resetModal">
      <i class="bi bi-trash me-1"></i>Resetuj bazu i storage
    </button>
  </div>
</div>
...
function _cls(line) {
  ...
  if (/Nova verzija|Amandman/.test(line)) return 'cron-new';
...
function startScraper() {
  ...
  $('#progBar').css('width', '0%').attr('class', 'progress-bar bg-primary');
  $('#progPct').text('0 / —');
  $('#progLabel').text('Pokreće se...');
  $('#sNova').text('0'); $('#sSkip').text('0'); $('#sErr').text('0');
  _setRunning(true);
  _addLine('--- Check pokrenut ' + new Date().toLocaleString('sr-Latn') + ' ---');

  _sse = new EventSource('../api.php?action=run_check_stream');
  ...
  _sse.onmessage = function(e) {
    ...
    if (d.line !== undefined) {
      ...
      if (totM) {
        ...
        $('#progLabel').text('Obrađuje 0 od ' + _total);
        return;
      }
      ...
      if (stM) {
        ...
        $('#progLabel').text('Obrađeno ' + done + ' od ' + tot);
      ...
      if (/▶ Nastavljam/.test(d.line)) {
        ...
        $('#progBar').attr('class', 'progress-bar bg-primary');
      }
      _addLine(d.line);
    }
...
async function stopScraper() {
  ...
  // SSE će se zatvoriti kada cron.php detektuje stop fajl i završi
}
...
function _onDone(ok) {
  ...
  if (_sse) { _sse.close(); _sse = null; }
  _running = false;
  $('#btnFirstRun').prop('disabled', false);
  $('#btnStop').prop('disabled', true)
              .html('<i class="bi bi-stop-fill"></i> <span>Zaustavi</span>');
  $('#scraperStatus')
